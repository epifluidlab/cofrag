...
cf_dist = function(file_name,
  ...
  # chr_name: "14"
  # bw_file_name: bigWig 
  # maps: mappability threshold
  # mapq: map quality threshold
  # cluster_core: number of logical job units
  # add_value: rounding-up to avoid zero values
  ...
  chr_bw_data = annoGR2DF(import(bw_file_name))
  # Filter out low mappability regions
  ...
  begin_position = 1
  i_p_value = NULL
  distance_matrix = foreach(
    i = 1:region_number,
    .combine = "cbind",
    .export = "data_filter",
    .packages = c("Rsamtools", "rtracklayer")
  ) %dopar% {
    i_p_value = NULL
    which = GRanges(Rle(chr_name), IRanges(((i - 1) * binsize + begin_position):(i *
                                                                                   binsize)))
    ...
    if (length(i_region_data$flag) == 0) {
      ...
      if (length(i_fragment_length) == 0) {
        ...
        for (j in 1:region_number) {
          which = GRanges(Rle(chr_name), IRanges(((j - 1) * binsize + begin_position):(j *
                                                                                         binsize)))
          p1 = ScanBamParam(which = which, what = what)
