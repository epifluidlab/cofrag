...
# Convert the mundane vanila distance matrix to n*3 sparse matrix representation
# Similar to the one of Hi-C
raw_cf_obs = function(cfDNA_distance_matrix,
                      Hic_obs_data,
                      binsize = 500000) {
  # Need validation for binsize - avoid discrepency
  # To match up the range of Hi-C data
...
correct_cf_obs = function(cfDNA_obs_data, Hic_obs_data, title_name) {
  ...
  for (i in 1:length(Hic_distance_kinds)) {
    ...
    Hic_distance_frequency = c(Hic_distance_frequency,
                               sum(Hic_obs_data$V3[i_location]) / total_frequency)
  }
  ...
  for (i in 1:length(cfDNA_distance_kinds)) {
    ...
    if (length(sum(cfDNA_obs_data$V3[j_location], na.rm = TRUE)) == 0) {
      ...
      cfDNA_distance_frequency = c(
        cfDNA_distance_frequency,
        sum(cfDNA_obs_data$V3[j_location], na.rm = TRUE) / total_frequency
      )
    }
  }
  ...
  for (i in 1:length(cfDNA_distance_kinds)) {
    ...
    if (length(sum(cfDNA_obs_data$V3[j_location], na.rm = TRUE)) == 0) {
      ...
      cfDNA_distance_frequency_new = c(
        cfDNA_distance_frequency_new,
        sum(cfDNA_obs_data$V3[j_location], na.rm = TRUE) / total_frequency
      )
    }
  }
  ...
  ggplot(plot_data,
         aes(x = interaction_Distance, y = fraction_of_interaction, group = kinds)) +
...
cf_mat = function(cfDNA_obs_data,
                  cfDNA_distance_matrix,
                  binsize = 500000) {
  cfDNA_corrected_data_source = rep(NA,
                                    ncol(cfDNA_distance_matrix) * ncol(cfDNA_distance_matrix))
  cfDNA_corrected_distance_matrix = matrix(
    cfDNA_corrected_data_source,
    ncol(cfDNA_distance_matrix),
    ncol(cfDNA_distance_matrix)
  )
